# PowerShell script to run RSVD Fisher/Hessian for regen_inception with 10k samples, k=500 (train)
...
Write-Host "Completed RSVD Hessian run (train) for $trainer with N=$numSamples, k=$k, useLabels=$useLabels. Log: $logPath"
