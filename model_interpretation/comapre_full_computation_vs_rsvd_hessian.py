...
def validate_rsvd_approximation(model_name):
    ...
    U_path = base_dir / f"train_{model_name}_U_rsvd.npy"
    S_path = base_dir / f"train_{model_name}_S_rsvd.npy"
    V_path = base_dir / f"train_{model_name}_V_rsvd.npy"
    ...
    # Calculate trace errors to compare with RSVD incremental error
    trace_full = np.trace(F_full)
    trace_approx = np.trace(F_approx)
    trace_error = trace_full - trace_approx
    ...
    print(f"Trace error: {trace_error:.6e}")
    
    # Load RSVD error bound if available
    rsvd_stats_file = base_dir / f"train_{model_name}_fisher_stats_rsvd.txt"
    rsvd_error_bound = None
    if rsvd_stats_file.exists():
        with open(rsvd_stats_file, 'r') as f:
            for line in f:
                if 'rsvd_error_bound' in line:
                    try:
                        rsvd_error_bound = float(line.split(':')[1].strip())
                        print(f"RSVD 'lost energy' error bound: {rsvd_error_bound:.6e}")
                    except:
                        pass
    ...
    # Use singular values as eigenvalues for RSVD approximation
    eigenvals_approx = np.abs(S)
    ...
    # Create the original comparison plot with ALL eigenvalues
    # Load ALL eigenvalues from full computation
    eigenvals_full_all = np.load(eigenvals_full_path)
    eigenvals_full_all = np.sort(eigenvals_full_all)[::-1]  # Sort descending
    
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(eigenvals_full_all) + 1), eigenvals_full_all, 'b-', label='Full Computation (All)', alpha=0.7)
    plt.plot(range(1, len(eigenvals_approx) + 1), eigenvals_approx, 'r-', label='RSVD', alpha=0.7)
    
    plt.xlabel('Eigenvalue Index')
    plt.ylabel('Eigenvalue')
    plt.yscale('log')
    plt.title(f'Eigenvalue Spectrum: {model_name} (Full Range)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Save the comparison plot (this should be the full range)
    comparison_plot_path = base_dir / f'{model_name}_eigenvalue_comparison.png'
    plt.savefig(comparison_plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Saved full range eigenvalue comparison plot to: {comparison_plot_path}")
    
    # Create cumulative sum plot
    plt.figure(figsize=(10, 6))
    
    # Calculate cumulative sum and normalize by total sum
    cumsum_eigenvals = np.cumsum(eigenvals_full_all)
    total_sum = np.sum(eigenvals_full_all)
    cumsum_normalized = cumsum_eigenvals / total_sum
    
    plt.plot(range(1, len(cumsum_normalized) + 1), cumsum_normalized, 'b-', linewidth=2, label='Cumulative Sum / Total Sum')
    
    # Add horizontal lines for reference
    plt.axhline(y=0.9, color='r', linestyle='--', alpha=0.7, label='90% of total')
    plt.axhline(y=0.95, color='orange', linestyle='--', alpha=0.7, label='95% of total')
    plt.axhline(y=0.99, color='green', linestyle='--', alpha=0.7, label='99% of total')
    
    # Add vertical line at k (RSVD components)
    plt.axvline(x=k, color='purple', linestyle=':', linewidth=2, label=f'RSVD components (k={k})')
    
    # Add vertical lines at energy thresholds
    for threshold, name, color in [(0.9, "90%", 'red'), (0.95, "95%", 'orange'), (0.99, "99%", 'green')]:
        idx = np.argmax(cumsum_normalized >= threshold)
        if cumsum_normalized[idx] >= threshold:
            plt.axvline(x=idx+1, color=color, linestyle=':', alpha=0.7, linewidth=1.5, 
                       label=f'{name} at {idx+1} components')
    
    plt.xlabel('Eigenvalue Index')
    plt.ylabel('Cumulative Sum / Total Sum')
    plt.title(f'Cumulative Eigenvalue Energy: {model_name}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.xlim(1, len(eigenvals_full_all))
    plt.ylim(0, 1.02)
    
    # Save the cumulative sum plot
    cumsum_plot_path = base_dir / f'{model_name}_eigenvalue_cumsum.png'
    plt.savefig(cumsum_plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Saved cumulative eigenvalue plot to: {cumsum_plot_path}")
    
    # Print cumulative statistics
    cumsum_at_k = cumsum_normalized[k-1] if k <= len(cumsum_normalized) else cumsum_normalized[-1]
    print(f"\nCumulative Energy Statistics:")
    print(f"Energy captured by first {k} components: {cumsum_at_k:.4f} ({cumsum_at_k*100:.2f}%)")
    
    # Find how many components needed for 90%, 95%, 99%
    for threshold, name in [(0.9, "90%"), (0.95, "95%"), (0.99, "99%")]:
        idx = np.argmax(cumsum_normalized >= threshold)
        if cumsum_normalized[idx] >= threshold:
            print(f"Components needed for {name} energy: {idx+1}")
    ...
    return {
        ...
        'rsvd_error_bound': rsvd_error_bound,
        'num_components': k,
