...
2. Project the Fisher matrix onto $\Omega$. Using the finite sample approximation:  
   ...
   Note that we never explicitly form the full Fisher matrix, instead we only have mxk projected matrix. 
...
4. Project the Fisher matrix onto $Q$:  
   ...
   again, we are left with only an mxk matrix.
...
2. Project Fisher matrix onto $\Omega$ to get $Y = F\Omega$
   - Computation: $ O(NC(O(gradient) + mk))$
   - Memory: O(mk) - only store the projected matrix Y
   - Easily parallelizable: Yes, both over samples and over coloums of $ \Omega$ 
...
4. Project Fisher onto Q to get B
   - Computation: $ O(NC(O(gradient) + mk))$ (similar to step 2)
   - Memory: O(mk) - only store the projected matrix Y
   - Easily parallelizable: Yes, both over samples and over coloums of $ \Omega$ 
   
   
5. SVD of small matrix B
   - Computation: $O(4mk^2)$ for full SVD of kxk matrix
   - Memory:  for matrices
   - Parallelization: ?

optional:

6. Final multiplication U = QÛ
   - Computation: O(mk^2)
